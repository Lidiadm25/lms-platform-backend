...
export class GradeService {
  ...
  async create(createGradeDto: CreateGradeDto, user: User) {
    ...
    if (user.id != task.task.user_author.id) {
      throw new UnauthorizedException(
        `Only the author of the task can put the grades`,
      );
    }
  ...
  async update(id: string, updateGradeDto: UpdateGradeDto) {
    const submit = await this.submitRepository.findOneBy({
      id: updateGradeDto.taskSubmitId,
    });

    if (
      updateGradeDto.taskSubmitId &&
      updateGradeDto.taskSubmitId !== submit!.id
    ) {
